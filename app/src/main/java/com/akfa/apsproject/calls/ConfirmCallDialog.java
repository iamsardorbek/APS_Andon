...
public class ConfirmCallDialog extends DialogFragment implements View.OnTouchListener{
    ...
    private int shopNo, equipmentNo, pointNo;
    ...
    private void initInstances(final View view) {
        ...
        shopNo = bundle.getInt("Номер цеха");
        equipmentNo = bundle.getInt("Номер линии");
        ...
        try {
            PointDataRetriever.fillInPointDataToTextViews(Objects.requireNonNull(getContext()), shopNameTextview, equipmentNameTextview, pointNoTextView, null, shopNo, equipmentNo, pointNo, -1);
            //здесь нету данных о подпункте, поэтому subpointTExtview = null, а subpointNo = -1
        }
...
//                        }
                        ...
                        //----считал дату и время----//
                        try {
                            callRef.setValue(new Call(dateCalled, timeCalled, UserData.login, whoIsCalled, shopNo, equipmentNo, pointNo, equipmentName, shopName, false));
                            Objects.requireNonNull(getDialog()).dismiss();
                        }
                        catch (NullPointerException npe) {
                            ExceptionProcessing.processException(npe, getResources().getString(R.string.program_issue_toast), getContext());
